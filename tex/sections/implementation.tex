...
%the fact that the \gls{opengl} world space coordinates uses a right-hand coordinate system while the \gls{opengl} \gls{ndc} space uses a left-hand coordinate system
%\gls{opengl} camera looks down along the negative Z axis (as seen in \cref{fig:ndc}), while in most computer vision libraries used to compute the camera intrinsic parameters, such as OpenCV calib3d module in which the camera looks along the positive Z axis (diagram in \cref{fig:camera-intrinsics}).

The correction of lens distortion for \gls{dlp} projectors, even thought being relatively small, can be done in a post-processing stage after 3D rendering, and typically uses 3 coefficients for removing radial distortions and 2 coefficients for accounting for the tangential distortions.


\begin{figure}[!ht]
	\centering
	\includegraphics[height=.17\textheight]{dlp-projector-diagram-dmd}
...
\begin{figure}[!ht]
	\centering
	\includegraphics[height=.22\textheight]{camera-intrinsics-2}
...
\begin{figure}[!ht]
	\centering
	\includegraphics[height=.15\textheight]{gl-projection-matrix}
	\caption[{\glsentrytext{opengl} perspective frustum in camera space (left) with the associated \glsentrytext{ndc} cube in clipping space}]{{\glsentrytext{opengl} perspective frustum in camera space (left) with the associated \glsentrytext{ndc} cube in clipping space}\protect\footnotemark}
	\label{fig:ndc}
\end{figure}

\begin{figure}[!ht]
	\centering
	\includegraphics[width=.99\textwidth]{similar-triangles}
...
\begin{figure}[!ht]
	\centering
	\includegraphics[height=.122\textheight]{object-reconstruction-front}
	\includegraphics[height=.122\textheight]{object-reconstruction-back}
...
%	\begin{floatrow}[2]
%		\ffigbox[\FBwidth]
%		{\includegraphics[height=.082\textheight]{mitsubishi-m000t20873-front}\includegraphics[height=.082\textheight]{mitsubishi-m000t20873-back}}
%		{\caption{Mitsubishi M000T20873 starter motor}\label{fig:starter-motor}}
%		\ffigbox[\FBwidth]
%		{\includegraphics[height=.082\textheight]{object-reconstruction-front}\includegraphics[height=.082\textheight]{object-reconstruction-back}}
